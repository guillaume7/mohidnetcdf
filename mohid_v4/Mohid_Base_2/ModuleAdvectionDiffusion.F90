...
cd1 :   if (ready_ == READ_LOCK_ERR_) then
    ...
    subroutine SetDischarges (AdvectionDiffusionID, DischFlow, DischConc,               &
                              DischI, DischJ, DischK, DischVert, DischNumber, IgnoreDisch, STAT)
...
din:    do n = 1, Me%ExternalVar%DischNumber  

            if (Me%ExternalVar%IgnoreDisch(n)) cycle      
            
            i  = Me%ExternalVar%DischI(n)
            j  = Me%ExternalVar%DischJ(n)
            kd = Me%ExternalVar%DischK(n)

            if (Me%ExternalVar%DischVert(n) == DischUniform_) then
