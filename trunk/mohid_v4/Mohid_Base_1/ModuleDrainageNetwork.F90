...
! XS_CALC                   : 1/2               [1]         !Method to compute trapezoidal cross section discretization:
!                                                           !1 - Analitic; 2 - Compute dh = (VolNew - VolOld) / AHOld
...
! OUTPUT_TIME               : int int...        [-]
! DOWNSTREAM_BOUNDARY       : int               [1]         !0 - Dam, 1 - Normal, 2 - ImposedWaterDepth
...
!   CROSS_SECTION_TYPE      : int               [1]         !1 - Trapezoidal
...
!   BOTTOM_LEVEL            : real              -           !Bottom level of cross section
...
Module ModuleDrainageNetwork
    ...
    private ::              InitializeCrossSection
    ...
    integer, parameter                              :: ImposedWaterDepth    = 2
    ...
    type       T_DrainageNetwork
        ...
        real,    dimension(:,:), pointer            :: ChannelsTopWidth     => null()
...
cd0 :   if (ready_ .EQ. OFF_ERR_) then
    ...
    subroutine ReadDataFile
        ...
        call ReadFileName('DRAINAGE_NETWORK_HDF', Me%Files%HDFFile,             &
                           ...
                           STAT = STAT_CALL)
        if (STAT_CALL /= SUCCESS_) stop 'ModuleDrainageNetwork - ReadDataFile - ERR02a' 
        ...
        !Method to compute trapezoidal cross section discretization
        ! 1 - Analitic
        ! 2 - Compute dh = (VolNew - VolOld) / AHOld
        call GetData(Me%XSCalc,                                                 &
                     Me%ObjEnterData, flag,                                     &  
                     keyword      = 'XS_CALC',                                  &
                     ClientModule = 'DrainageNetwork',                          &
                     SearchType   = FromFile,                                   &
        ...
        call GetData(Me%NumericalScheme,                                        &
                      ...
                      STAT         = STAT_CALL)                                  
        if (STAT_CALL /= SUCCESS_) stop 'ModuleDrainageNetwork - ReadDataFile - ERR09a'        
        ...
        call GetData(Me%GlobalManning,                                          &
                     ...
                     Default      = null_real,                                  &
                     STAT         = STAT_CALL)                                  
        if (STAT_CALL /= SUCCESS_) stop 'ModuleDrainageNetwork - ReadDataFile - ERR10'        
        ...
        call GetData(Me%MinimumSlope,                                           &
                     ...
                     Default      = 0.0,                                        &
                     STAT         = STAT_CALL)                                  
        if (STAT_CALL /= SUCCESS_) stop 'ModuleDrainageNetwork - ReadDataFile - ERR10a'        
        ...
        if (Me%MinimumWaterDepth.LT.0.0) then
            ...
            stop 'ModuleDrainageNetwork - ReadDataFile - ERR08'
        end if
        ...
        if (Me%MinimumWaterDepthProcess.LT.0.0) then
            ...
            stop 'ModuleDrainageNetwork - ReadDataFile - ERR08a'
        end if
        ...
        if (Me%Continuous) then
            ...
            call GetData(Me%StopOnWrongDate,                                        &
                         ...
                         Default      = .true.,                                     &
                         STAT         = STAT_CALL)                                  
            if (STAT_CALL /= SUCCESS_) stop 'ModuleDrainageNetwork - ReadDataFile - ERR14'        
            ...
            if (Me%InitialWaterDepth.LT.0.0) then
                ...
                stop 'ModuleDrainageNetwork - ReadDataFile - ERR09'
            end if
        ...
        call GetData(Me%Stabilize,                                              &
                     ...
                     Default      = .false.,                                    &
                     STAT         = STAT_CALL)                                  
        if (STAT_CALL /= SUCCESS_) stop 'ModuleDrainageNetwork - ReadDataFile - ERR16'        
        ...
        if (Me%AerationEquation /= PoolAndRifle_ .and. Me%AerationEquation /= ChannelControled_) then
            ...
            stop 'ModuleDrainageNetwork - ReadDataFile - ERR21a'  
        endif
        ...
        call GetData(Me%T90Var_Method,                                      &   
                     ...
                     Default        = Canteras,                             &
                     STAT           = STAT_CALL)              
        if (STAT_CALL .NE. SUCCESS_) stop 'ModuleDrainageNetwork - ReadDataFile - ERR22' 
        ...
        call GetData(Me%ComputeOptions%CalcFractionSediment,                &
                     ...
                     ClientModule = 'DrainageNetwork',                      &
                     STAT         = STAT_CALL)
        if (STAT_CALL /= SUCCESS_) stop 'ModuleDrainageNetwork - ReadDataFile - ERR30_Wassim_FractionSediment' 
        ...
        call GetData(AuxString, Me%ObjEnterData,  flag,                     &
                     ...
                     Default        = 'SUM',                                &                         
                     STAT           = STAT_CALL)
        if (STAT_CALL .NE. SUCCESS_) stop 'ModuleDrainageNetwork - ConstructDownstreamBoundary - ERR02'
        ...
        select case (trim(adjustl(AuxString)))
            ...
            case default
                ...
                stop 'ModuleDrainageNetwork - ReadDataFile - ERR32' 
        end select
    ...
    subroutine ConstructDownstreamBoundary
        ...
        if (Me%Downstream%Boundary /= Normal .and. Me%HydrodynamicApproximation == KinematicWave) then
            write(*,*) 
            write(*,*) 'DOWNSTREAM_BOUNDARY = ',  Me%Downstream%Boundary
            write(*,*) 'Impossible to compute without backwater effects.'
            write(*,*) 'Requires HYDRODYNAMIC_APROX > 1'
            stop 'ModuleDrainageNetwork - ConstructDownstreamBoundary - ERR01a'
        endif

if1:    if (Me%Downstream%Boundary == ImposedWaterDepth) then
            ...
            select case (trim(adjustl(AuxString)))
                ...
                case default
                    ...
                    stop 'ModuleDrainageNetwork - ConstructDownstreamBoundary - ERR03'
            end select
            ...
            call GetData(Me%Downstream%DefaultValue, Me%ObjEnterData,  flag,    &
                         ...
                         STAT           = STAT_CALL)
            if (STAT_CALL .NE. SUCCESS_) stop 'ModuleDrainageNetwork - ConstructDownstreamBoundary - ERR04'


            if (flag == 0) then
                write(*,*)'Please define default value for water depth at downstream boundary'
                stop 'ModuleDrainageNetwork - ConstructDownstreamBoundary - ERR05'
            end if
...
if2:        if (Me%Downstream%Evolution == ReadTimeSerie) then
                ...
                call GetData(Me%Downstream%FileName,                             &
                             ...
                             ClientModule = 'DrainageNetwork',                  &
                             STAT         = STAT_CALL)                                      
                if (STAT_CALL .NE. SUCCESS_) stop 'ModuleDrainageNetwork - ConstructDownstreamBoundary - ERR06'

                if (flag==0)then
                    write(*,*)'Time Serie File Name not given'
                    stop 'ModuleDrainageNetwork - ConstructDownstreamBoundary - ERR07'
                endif
                ...
                call GetData(Me%Downstream%DataColumn,                           &
                             ...
                             ClientModule = 'DrainageNetwork',                  &
                             STAT         = STAT_CALL)                                      
                if (STAT_CALL .NE. SUCCESS_) stop 'ModuleDrainageNetwork - ConstructDownstreamBoundary - ERR08'

                if (flag==0)then
                    write(*,*)'Data Column not given'
                    stop 'ModuleDrainageNetwork - ConstructDownstreamBoundary - ERR09'
                endif
                ...
                call StartTimeSerieInput(Me%Downstream%ObjTimeSerie,            &
                                         ...
                                         Me%ObjTime,                            &
                                         STAT = STAT_CALL)
                if (STAT_CALL .NE. SUCCESS_) stop 'ModuleDrainageNetwork - ConstructDownstreamBoundary - ERR10'
...
ifXS:   if (NewNode%CrossSection%Form == Trapezoidal) then
            ...
            if (flag /= 1) then
                ...
                stop 'ModuleDrainageNetwork - ConstructNode - ERR10'
            endif
            ...
            if (flag /= 1 ) then
                ...
                stop 'ModuleDrainageNetwork - ConstructNode - ERR12'
            endif
            ...
            if (flag /= 1) then
                ...
                stop 'ModuleDrainageNetwork - ConstructNode - ERR14'
            endif
            ...
            call GetData(NewNode%CrossSection%BottomLevel,                      &
                         Me%Files%ObjEnterDataNetwork, flag,                    &  
                         keyword      = 'BOTTOM_LEVEL',                         &
                         ClientModule = 'DrainageNetwork',                      &
                         SearchType   = FromBlock,                              &
                         STAT         = STAT_CALL)                                      
            if (STAT_CALL .NE. SUCCESS_) stop 'ModuleDrainageNetwork - ConstructNode - ERR15'
            if (flag /= 1) then
                write (*,*)'Invalid Node Bottom Level [BOTTOM_LEVEL]'
                stop 'ModuleDrainageNetwork - ConstructNode - ERR16'
            endif
            ...
            stop 'ModuleDrainageNetwork - ConstructNode - ERR17'
        ...
        if (.not. Me%Continuous) then                            
            ...
            if (STAT_CALL .NE. SUCCESS_)                                        &
                    stop 'ModuleDrainageNetwork - ConstructNode - ERR20.'   
...
ifB:    if (NewProperty%ComputeOptions%BottomFluxes) then
    ...
    subroutine InitializeVariables
        ...
        if (Me%HasGrid) then
            ...
            allocate(Me%ChannelsTopWidth      (Me%Size%ILB:Me%Size%IUB, Me%Size%JLB:Me%Size%JUB))
            ...
            do NodeID = 1, Me%TotalNodes
                ...
                Me%ChannelsTopWidth     (CurrNode%GridI, CurrNode%GridJ) = CurrNode%CrossSection%TopWidth
    ...
    subroutine InitializeNodes
        ...
        do NodeID = 1, Me%TotalNodes    
            ...
            call InitializeCrossSection (CurrNode)
...
if1:        if (CurrNode%nDownstreamReaches /= 0) then
                ...
                if (CurrNode%CrossSection%Form .EQ. Trapezoidal) then            
                    ...
                    stop 'Tabular Cross Section - InitializeNodes - ModuleDrainageNetwork - ERR02' 
                end if
            ...
            if (.not. Me%Continuous) CurrNode%WaterLevel = CurrNode%WaterDepth + CurrNode%CrossSection%BottomLevel
    ...
    subroutine InitializeCrossSection (CurrNode)
        ...
        if (CurrNode%nDownstreamReaches /= 0) then
            ...
            CurrNode%VerticalArea = ( CurrNode%CrossSection%BottomWidth &
                                  + CurrNode%CrossSection%Slope         &
                                  * CurrNode%WaterDepth )               &
                                  * CurrNode%WaterDepth
    ...
    end subroutine InitializeCrossSection
...
if2:        if (Me%TimeSerie%ByNodes) then
                ...
                do i = 1, Me%TimeSerie%nProp
                    ...
                    if (Me%TimeSerie%Name (i) == Char_Flow .or. Me%TimeSerie%Name (i) == Char_Velocity) then
...
if0:    if (Me%TimeSerie%nProp .GT.BaseTimeSeries) then
    ...
    subroutine ConstructLog
        ...
        else if (Me%HydrodynamicApproximation == DynamicWave) then
        ...
        if (Me%NumericalScheme == ExplicitScheme) then
            ...
            write(*, *)"Numerical Scheme     : INVALID"
            stop 'ModuleDrainageNetwork - ConstructLog - ERR02'
        end if

        
        !Writes Downstream Boundry
        if (Me%Downstream%Boundary == Normal) then
            write(*, *)"Downstream Boundary  : Normal"
        else if (Me%Downstream%Boundary == ImposedWaterDepth) then
            write(*, *)"Downstream Boundary  : Imposed Water Depth"
        else if (Me%Downstream%Boundary == Dam) then
    ...
    subroutine GetChannelsTopWidth (DrainageNetworkID, ChannelsTopWidth, STAT)

        !Arguments--------------------------------------------------------------
        integer                                         :: DrainageNetworkID
        real, dimension (:,:), pointer                  :: ChannelsTopWidth
        integer, intent(OUT), optional                  :: STAT
        ...
        if ((ready_ .EQ. IDLE_ERR_     ) .OR.                                   &
            ...
            call Read_Lock(mDRAINAGENETWORK_, Me%InstanceID)
            ChannelsTopWidth => Me%ChannelsTopWidth
    ...
    end subroutine GetChannelsTopWidth
...
if1:        if (DownNode%nDownstreamReaches .EQ. 0) then
                ...
                !Outlets
                if (Me%Downstream%Boundary == Dam ) then 
        
                    CurrReach%FlowNew   = 0.0
                    CurrReach%Velocity  = 0.0
        
                elseif (Me%Downstream%Boundary == ImposedWaterDepth) then 
                    ...
                    if (Me%HydrodynamicApproximation == DiffusionWave) then
                        ...
                        do iReach = 1, UpNode%nUpStreamReaches
                            UpReach   => Me%Reaches (UpNode%UpstreamReaches (iReach))
                            CurrReach%FlowNew = CurrReach%FlowNew + UpReach%FlowOld
                        enddo
...
if1:    if (CurrNode%nDownstreamReaches /= 0) then
            ...
            if (CurrNode%VolumeNew > PoolVolume) then
                ...
                Av_New  = (CurrNode%VolumeNew - PoolVolume) / CurrNode%Length

                if (CurrNode%CrossSection%Slope .LE. AllmostZero) then                   
                    !Rectangular
                    h_New = Av_New / CurrNode%CrossSection%BottomWidth                  
                else

                    binomio = CurrNode%CrossSection%BottomWidth**2.             &
                            + 4. * CurrNode%CrossSection%Slope * Av_New

                    if (binomio .LT. 0.0) then
                        stop 'ComputeCrossSection - ModuleDrainageNetwork - ERR01'
                    ...
                    h_New  = ( - CurrNode%CrossSection%BottomWidth + sqrt_binomio)  &
                           / (2. * CurrNode%CrossSection%Slope)
                ...
                Av_New                  = max(Av_New - Me%MinimumWaterDepth * CurrNode%CrossSection%BottomWidth, 0.0)
                                                  
                CurrNode%VerticalArea   = Av_New 
    ...
    subroutine ComputeKinematicWave (CurrReach)
        ...
        if (CurrReach%VerticalArea > AllmostZero) then
            CurrReach%FlowNew  = CurrReach%VerticalArea * CurrReach%HydraulicRadius **(2.0/3.0) &
                                 * sqrt(CurrReach%Slope)  / CurrReach%Manning
...
if1:    if (Me%HydrodynamicApproximation == DynamicWave) then
           ...
           !Down Flux
            ...
            Advection = (AdvectionUp - AdvectionDown) * DT / CurrReach%Length
...
if2:            if (Me%TimeSerie%nProp .GT.BaseTimeSeries) then
...
do1:    do i = 1, BaseTimeSeries
            ...
            if (Me%TimeSerie%Name (i) == Char_WaterDepth) then
            ...
            else if (Me%TimeSerie%Name (i) == Char_WaterLevel) then
            ...
            else if (Me%TimeSerie%Name (i) == Char_PercentageMaxVolume) then     
            ...
            else if (Me%TimeSerie%Name (i) == Char_VerticalArea) then
            ...
            else if (Me%TimeSerie%Name (i) == Char_FlowToChannels) then
            ...
            else if (Me%TimeSerie%Name (i) == Char_Volume) then
            ...
            else if (Me%TimeSerie%Name (i) == Char_Flow) then  
             ...
             else if (Me%TimeSerie%Name (i) == Char_Velocity) then
            ...
            else if (Me%TimeSerie%Name (i) == Char_GWFlowToChannels) then
            ...
            else if (Me%TimeSerie%Name (i) == Char_PoolDepth) then
            ...
            else if (Me%TimeSerie%Name (i) == Char_DT) then
            ...
            else if (Me%TimeSerie%Name (i) == Char_DTLocal) then
        ...
        !Properties-------------------------------------------------------------
if1:    if (Me%TimeSerie%nProp .GT.BaseTimeSeries) then
