...
i1:         if (ComputePoints(i-1) == Compute .and. ComputePoints(i) == Compute) then
                ...
                call ComputeAdvectionFace(Prop4, V2, du4, dt, QFace, VolumeRelMax,         &
                                          Method, TVD_Limitation, NearBoundary, Upwind2, CFace)

                CrLeft  = QFace * dt / V2(1)
                CrRight = QFace * dt / V2(2)
...
i1:         if (ComputePoints(i-1) == Compute .and. ComputePoints(i) == Compute) then
    ...
    subroutine ComputeAdvectionFace(Prop, V, du, dt, QFace, VolumeRelMax,                &
        ...
        if (VolumeRel < 1) VolumeRel = 1. / VolumeRel

        if (QFace > 0) then
            Cr  = Courant (QFace,V(1),dt)  
        else
