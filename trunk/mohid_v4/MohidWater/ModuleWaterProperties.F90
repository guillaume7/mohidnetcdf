...
do1 :   do while (associated(PropertyX))
        ...
        if (Me%Coupled%Discharges%Yes) then
            ...
            if (DischargesID == 0)  then                                                
                ...
                call GetDischargesNumber(Me%ObjDischarges, ND, STAT=STAT_CALL)
                
                if (STAT_CALL /= SUCCESS_)      &
                    stop 'Construct_Sub_Modules - ModuleWaterProperties - ERR50'

                Me%Discharge%Number = ND

                allocate(Me%Discharge%Flow   (ND),                                      &
                         Me%Discharge%i      (ND),                                      &
                         Me%Discharge%j      (ND),                                      &
                         Me%Discharge%k      (ND),                                      &
                         Me%Discharge%Vert   (ND),                                      &
                         Me%Discharge%Ignore (ND))
                ...
                PropertyX   => Me%FirstProperty
                do while (associated(PropertyX))
                    if (PropertyX%Evolution%Discharges .and. ND > 0) then
                
                        allocate(PropertyX%DischConc(ND))

                        PropertyX%DischConc(ND) = FillValueReal
        ...
        if (Me%Coupled%DataAssimilation%Yes .or. Me%Coupled%AltimetryAssimilation%Yes) then
            ...
            if(AssimilationID == 0)then
                ...
                call StartAssimilation(Me%ObjAssimilation,                   &
                                       ...
                                       Me%ObjGeometry,                       &
                                       STAT = STAT_CALL)
                if (STAT_CALL /= SUCCESS_) stop 'Construct_Sub_Modules - WaterProperties - ERR60'
        ...
        do while (associated(PropertyX))
            ...
            if (PropertyX%Statistics) then
                ...
                call ConstructStatistic (StatisticID      = PropertyX%StatisticID,      &
                                         ...
                                         STAT             = STAT_CALL)
                if (STAT_CALL /= SUCCESS_) stop 'Construct_Sub_Modules - ModuleWaterProperties - ERR70'
...
cd1 :   if      (STAT_CALL .EQ. FILE_NOT_FOUND_ERR_   ) then
   ...
   !This subroutine set the evolutions options of a decay rate property.           
    ...
    subroutine SetDecayRatePropertyOptions(NewProperty)
        ...
        !if the T90 is defined in hours in the input data files must be convert into seconds
        if (NewProperty%Evolution%T90Hours) then
            NewProperty%Concentration(:,:,:) = NewProperty%Concentration(:,:,:) * 3600.
        endif
        !----------------------------------------------------------------------
...
cd10:                       if (Property%evolution%Advec_Difus_Parameters%ImplicitH_Direction &
                    ...
                    if (Property%Evolution%Discharges)then
                        ...
                        call SetDischarges (Me%ObjAdvectionDiffusion, Me%Discharge%Flow,&
                                            ...
                                            Me%Discharge%Ignore,      STAT = STAT_CALL)
                    if (STAT_CALL .NE. SUCCESS_)                                            &
                ...
                if(Me%ExternalVar%Now .ge. PropertyX%Evolution%NextCompute)then
                    ...
                    if (PropertyX%ID%SolutionFromFile) then
                        ...
                        if (PropertyX%Evolution%T90Hours) then
                            PropertyX%Concentration(:,:,:) = PropertyX%Concentration(:,:,:) * 3600.
                        endif                        
...
dd:     do dis = 1, Me%Discharge%Number
            ...
            if (STAT_CALL /= SUCCESS_) stop 'ModifySpecificHeat - ModuleWaterProperties - ERR75'
            ...
            call GetDischargeFlowDistribuiton(Me%ObjDischarges, dis, FlowDistribution, &
                                              nCells, VectorI, VectorJ, VectorK, STAT = STAT_CALL)             
...
dn:         do n=1, nCells
                ...
                Me%Discharge%Flow   (dis) = AuxFlowIJ
                Me%Discharge%i      (dis) = i
                Me%Discharge%j      (dis) = j
                Me%Discharge%k      (dis) = k
 ...
 i2:                        if (ByPassON) then
                                ...
                                PropertyX%DischConc(dis) = PropertyX%Concentration(ib, jb, k)
                                ...
                                PropertyX%DischConc(dis) = DischargeConc
...
TOut:   if (Actual >= Me%OutPut%OutTime(OutPutNumber)) then
            ...
            if (Me%EndTime == Me%OutPut%OutTime(OutPutNumber)) then
...
First:              if (FirstTime) then 
                        ...
                        call HDF5WriteData  (Me%ObjHDF5, "/Results/density", "density",    &
            ...
            if (Me%Coupled%MacroAlgae%Yes)then
                ...
                call HDF5WriteData  (Me%ObjHDF5, "/Results/"//"macroalgae distribution",    &
        ...
        if (Actual >= Me%OutPut%SurfaceOutTime(SurfaceOutPutNumber)) then
            ...
            do while (associated(PropertyX))
                ...
                if(PropertyX%OutputSurfaceHDF)then
                    ...
                    !if the T90 is defined in hours in the input data files must be convert into seconds
                    if (PropertyX%Evolution%T90Hours) then
                        allocate(Array3D(ILB:IUB,JLB:JUB,KUB:KUB))
                        Array3D(:,:,KUB) = PropertyX%Concentration(:,:,KUB) * 3600.
                    else
                        Array3D => PropertyX%Concentration
                    endif
    ...
    subroutine OutPutHDF_AditionalFields (OutPutNumber)
        ...
        call SetMatrixValue(Aux3D, Me%Size, 0.)
...
i3:         if (me%DoSatType.eq.Apha) then
                ...
                do i=WILB, WIUB
                    ...
                    if (Me%ExternalVar%OpenPoints3D(i, j, WKUB) == OpenPoint) then
                        ...
                        do k = kbottom, WKUB
                            ...
                            !New Concentration
                            Aux3D(i, j, k) = PropOxygen%Concentration(i, j, k) / DOSAT * 100.
            ...
            call HDF5WriteData  (Me%ObjHDF5, "/Results/"//PropName, PropName,'%', Array3D = Aux3D, &
                                 OutputNumber = OutPutNumber, STAT = STAT_CALL)
...
i2:     if (Me%OutPut%Radiation) then 
            ...
            call HDF5WriteData  (Me%ObjHDF5, "/Results/"//PropName, PropName,'W/m2', Array3D = ShortWaveAverage, &
                                 OutputNumber = OutPutNumber, STAT = STAT_CALL)
            ...
            call HDF5WriteData  (Me%ObjHDF5, "/Results/"//PropName, PropName,'1/m', Array3D = ShortWaveExtinctionField, &
                                 OutputNumber = OutPutNumber, STAT = STAT_CALL)
        ...
        do while (associated(PropertyX))
            if (PropertyX%TimeSerie) then
                ...
                call WriteTimeSerie(Me%ObjTimeSerie,                                     &
                                    Data3D = PropertyX%Concentration, factor = factor, STAT = STAT_CALL)
                if (STAT_CALL /= SUCCESS_)                                               &
