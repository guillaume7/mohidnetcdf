...
em0:        do em =1, Nmodels            
            ...
            if (STAT_CALL /= SUCCESS_) stop 'ConstructLagrangianGlobalGlobal - ModuleLagrangianGlobal - ERR13'
            ...
            !External Modules
                ...
                if (em ==1) then
                    ...
                    call GetComputeTimeLimits(Me%ExternalVar%ObjTime,                   &
                                              ...
                                              STAT = STAT_CALL)
                    if (STAT_CALL /= SUCCESS_) stop 'ConstructLagrangianGlobalGlobal - ModuleLagrangianGlobal - ERR02'

                    ! Actualized the time
                    call GetComputeCurrentTime(Me%ExternalVar%ObjTime,                            &
                                               Me%ExternalVar%Now, STAT = STAT_CALL)                    
                    if (STAT_CALL /= SUCCESS_) stop 'ConstructLagrangianGlobalGlobal - ModuleLagrangianGlobal - ERR03'
                ...
                call GetGeometrySize(Me%EulerModel(em)%ObjGeometry,                                  &
                                     ...
                                     STAT = STAT_CALL)
                if (STAT_CALL /= SUCCESS_) stop 'ConstructLagrangianGlobalGlobal - ModuleLagrangianGlobal - ERR04'
...
d2:     do em =1, Me%EulerModelNumber
            ...
            if (Me%State%Filtration)  then
                ...
                Me%NextFiltration = Me%ExternalVar%Now
...
!        endif ic
    ...
    !                                        Value1, Time2, Value2, CurrentOrigin%Flow)
                ...
                StartTime = Me%ExternalVar%Now 
                EndTime   = Me%ExternalVar%Now + CurrentOrigin%DT_EMIT
...
iON:        if (CurrentOrigin%EmissionON) then
                ...
                case (Instantaneous_)
                    ...
                    if (.not. CurrentOrigin%Old) then
                        ...
                        if (Me%ExternalVar%Now >= CurrentOrigin%InstantEmission .and. CurrentOrigin%OnlyOnceEmit) then
...
i1:     if (Me%ExternalVar%Now >= Me%NextFiltration) then
...
BoxOr:  do while (associated(BoxOrigin))
            ...
            if (BoxOrigin%KillPartInsideBox) then
                ...
                if (Me%ExternalVar%Now >= BoxOrigin%InstantEmission .and. BoxOrigin%OnlyOnceEmit) then
                ...
                do while (associated(CurrentOrigin))
                    ...
                    do while (associated(CurrentProperty))
                        ...
                        if (CurrentProperty%Name == Me%MonitorProperty) then
                            ! The Mass Account is different for fecal coliforms, because usually initial concentration
                            ! is not in S.I. (usually is UFC / 100 ml). so, the mass calculation (C * V) is multiplied 
                            ! by 10
                            if (CurrentProperty%ID == Fecal_Coliforms_ .or.CurrentProperty%ID == E_Coli_) then
                                ...
                                do while (associated(CurrentPartic))
                                    ...
                                    if (Box > 0 .and. Me%EulerModel(em)%OpenPoints3D(i, j, k) == OpenPoint) then
                                        ...
                                        Me%EulerModel(em)%Monitor%InstMassByOrigin (Box, CurrentOrigin%ID) + &
                                        (10 * CurrentPartic%Concentration(nProp) * dble(CurrentPartic%Geometry%Volume)) 
                                    endif
